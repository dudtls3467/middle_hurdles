...
@WebServlet("/owner/restaurantInsert.do")
@MultipartConfig
public class RestaurantInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String[] requiredParams = {"restaurantNo", "localNo", "restaurantCodeNo"};
		...
		if (result == 1) {
			resp.sendRedirect(req.getContextPath() + "/admin/restrauntDetail.do?restaurantNo=" + resNo);
			return;
		}

		// 실패 시 지금 화면 다시 재시작 (값도 넣어주고)
		req.setAttribute("restaurant", newRes);
		resp.sendRedirect(req.getContextPath() + "/owner/restaurantInsert.do");
